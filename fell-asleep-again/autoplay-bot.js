...
(function () {
  ...
  // 體貼版：每一題預設選「a」，例外改選別的代號。這份表是全題庫重新逐題判斷過的結果
  // （完全沒看題庫裡「對方後續回覆」欄位），依六條紅旗準則排除：①要他解釋/計算/報告
  // 自己（例如「算給我聽」「我不打斷」）②交辦一件差事給他（例如「你把照片挑一下」）
  // ③預設懷疑、要他自證的條件式測試（例如「你講得出用途我就不說話」）④翻舊帳
  // （「上次你也是……結果……」）⑤敷衍打發 ⑥明著羞辱/威脅分手（這類屬於直女版，不會選
  // 進體貼版）。經玩家實際回報＋/bf/hell 實跑資料反覆校正過，也拿今晚睡沙發？的作法對照過
  // （校正過程跟每一輪的判斷理由都記在 notes.md，之後如果又抓到聊死題目可以繼續補）。
  ...
  // 直女版：每一題預設選「d」，例外改選別的代號——判斷「直女會忍不住講出口」的最嗆選項
  // （通常是明確羞辱/翻舊帳/威脅分手那句），找不到夠嗆的選項時退而求其次選次尖銳的。
  ...
  // 拿 /bf/hell 實跑兩局校正時發現的另一種扣分型態：字面看起來體貼／講道理，其實是要對方
  // 「交代、計算、報告」給她聽的句型——即使沒有半個難聽字，無聊指數照樣漲得比敷衍還兇
  // （例如「你把你算的講一遍，我聽，我不打斷」「那你算給我聽，下午還剩幾小時」「那你也去
  // 按一下我剛發的那篇，比較公平」）。這類「要求對方自證/表態/公平計較」的語氣扣中等分數，
  // 跟敷衍（TENDER_NEG）分開列，因為它不是不投入，是投入錯方向。
  ...
  // 直女版方向：嗆人/羞辱/翻舊帳 加分；具體接話/正面延續 扣分（近似做法：跟體貼版反著走，
  // 再額外拉高「明顯羞辱字眼」的權重，因為那類選項才是真正的直女嗆法）。
