...
(function () {
  ...
  // 體貼版：每一題選「a」——這是我逐題讀過全部 192 題的情境與選項文字（完全沒看題庫裡
  // 對方回覆/地雷欄位）之後，自己判斷求生欲最高的答案；核對過結算頁「戰績」揭曉的兩題
  // （冰箱食物、一個人打掃）都跟這個規則一致，所以用它當比對依據。
  //
  // 直男版：逐題另外判斷「一般直男會怎麼回」——特徵是回答字面問題而不是回應情緒、用邏輯
  // /事實/技術理由取代安慰、容易漏接暗示、拖延耍廢，但不是存心刻薄（存心刻薄的選項不選）。
  // 沒有固定代號規律，所以逐題記錄選哪一個代號。
  const BLUNT_LETTER = {
  ...
  // 直男版方向：字面/邏輯/事實/技術理由 加分；道歉/安慰/主動記細節/主動示愛 扣分
  // （近似做法：直接把體貼版的方向反過來，畢竟兩者本來就是同一光譜的兩端）。
